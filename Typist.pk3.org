...
#+begin_src c :tangle build/Typist.pk3/zscript/lesson/tt_selected_question_source.zs
...
class tt_HastyQuestionAnswerMatcher : tt_OriginSource
{
                                      ...
                                      Actor pawn)
  {
    ...
    for (uint i = 0; i < nTargets; ++i)
    {
      ...
      if (!question.isRight(answerString) || !isVisible(target, pawn))
      {
        continue;
      }
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_cvar.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_command_settings.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_command_settings_impl.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_settings_impl.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_sound_settings.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/settings/tt_sound_settings_impl.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/target_widget/tt_visibility_filter.zs
...
class tt_InfoPanel : tt_View
{
  ...
  {
    int     xStart   = Screen.getWidth() / 2;
    vector2 position = (xStart, Y_START);

    drawCommands(position);
    // TODO: make y shift more robust:
    drawTargets (position + (0, NewSmallFont.getHeight() * 3));
  }
  ...
  private ui void drawTargets(vector2 xy)
  {
    ...
    // 1. Find the first visible target.
    uint firstVisibleTargetIndex = findFirstVisibleTarget(player, targets);
    if (firstVisibleTargetIndex == targetCount) return;

    int xRight;
    int xLeft;
    // 2. Draw the first target in the center.
    {
      let    target         = targets.at(firstVisibleTargetIndex);
      let    question       = target.getQuestion();
      ...
      int targetWidth = tt_Drawing.getWidthForTarget(questionString,
                                                     hintedAnswer,
                                                     scale);
      xRight = halfScreen + targetWidth / 2 + HORIZONTAL_MARGIN + tt_Drawing.FRAME;
      xLeft  = halfScreen - targetWidth / 2 - HORIZONTAL_MARGIN + tt_Drawing.FRAME;
    }

    // 2. Draw the targets to the right while there is space.
    uint i = firstVisibleTargetIndex + 1;
    for (; i < targetCount; ++i)
    {
      let target = targets.at(i);
    ...
    // 3. Draw the targets to the left while there is space.
    for (; i < targetCount; ++i)
    {
      let target = targets.at(i);
  ...
  private ui uint findFirstVisibleTarget(PlayerPawn player, tt_KnownTargets targets)
  {
    let targetCount = targets.size();

    for (uint i = 0; i < targetCount; ++i)
      if (isVisible(player, targets.at(i).getTarget().getActor())) return i;
  ...
  private ui void drawCommands(vector2 xy)
  {
    ...
    for (uint i = 0; i < nCommands; ++i)
    {
      ...
      x -= targetWidth + HORIZONTAL_MARGIN;
    }
  }
