...
To accomodate the slower pace of the game, the game is automatically changed:
- enemies move slower
...
- Ctrl-Backspace to erase all input
...
This way, you'll waste no time switching from mouse to keyboard and back, and your
fingers are almost at the right typing position.

** Compatibility

Typist.pk3 isn't coded specifically for any game, so there is a chance it is
compatible with many GZDoom/UZDoom mods and games.
...
- Guncaster and Guncaster Vindicated are not compatible with Typist.pk3. Reason:
  ...
  modifications in Guncaster or GZDoom.
