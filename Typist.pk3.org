...
class tt_InfoPanel : tt_View
{
  ...
  override void draw(RenderEvent _)
  {
    ...
    for (; i < targetCount; ++i)
    {
      ...
      if (xRight + width > screenWidth) break;
...
class tt_TargetOverlay : tt_View
{
  ...
  override void draw(RenderEvent event)
  {
    ...
    for (uint i = 0; i < nWidgets; ++i)
    {
      ...
      int x = int(clamp(position.x - width / 2, 0, screenWidth  - width));
      int y = int(clamp(position.y - height,    0, screenHeight - height * 2));
