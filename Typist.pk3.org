...
class tt_PlayerSupervisor : tt_PlayerHandler
{
  ...
  private static tt_Aimer makeAimer(tt_OriginSource targetOriginSource,
  ...
  private static tt_QuestionSource makeQuestionSource(tt_Settings settings,
                                                      tt_PlayerSource playerSource)
  {
    ...
    let customLesson   = tt_StringSet.of("typist_custom_text");
  ...
  private static void makeCommands(tt_PlayerSource    playerSource,
  ...
  private static tt_OriginSource makeTargetOriginSource(
    tt_KnownTargetSource targetSource,
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/dasher.zs

// This class implements tt_Activatable by performing a dash.
class tt_Dasher : tt_Activatable abstract
{
  abstract double getDashAngle() const;
...
{
  static tt_DasherMock of() { return new("tt_DasherMock"); }

  override double getDashAngle()
  {
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/forward_dasher.zs

// This class implements tt_Dasher by specifying dash angle.
class tt_ForwardDasher : tt_Dasher
{
  static tt_ForwardDasher of(tt_PlayerSource playerSource, tt_CommandSettings settings)
  {
    let result = new("tt_ForwardDasher");
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/back_dasher.zs

// This class implements tt_Dasher by specifying dash angle.
class tt_BackDasher : tt_Dasher
{
  static tt_BackDasher of(tt_PlayerSource playerSource, tt_CommandSettings settings)
  {
    let result = new("tt_BackDasher");
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/left_dasher.zs

// This class implements tt_Dasher by specifying dash angle.
class tt_LeftDasher : tt_Dasher
{
  static tt_LeftDasher of(tt_PlayerSource playerSource, tt_CommandSettings settings)
  {
    let result = new("tt_LeftDasher");
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/right_dasher.zs

// This class implements tt_Dasher by specifying dash angle.
class tt_RightDasher : tt_Dasher
{
  static tt_RightDasher of(tt_PlayerSource playerSource, tt_CommandSettings settings)
  {
    let result = new("tt_RightDasher");
...
// This class contains Activatables and activates() ones with commands matching answer.
class tt_CommandDispatcher : tt_Activatable
{
  ...
  private play bool tryActivate(tt_Activatable activatable, string answer)
  {
    let commands = activatable.getCommands();
  ...
  private string makePrefixedCommand(String command)
  {
...
#+begin_src c :noweb-ref test-body
{
  ...
  env.settings.expect_getCommandPrefix("");
...
}
{
  ...
  env.settings.expect_getCommandPrefix("", 4);
...
class tt_CommandDispatcherTestEnvironment
{
  static tt_CommandDispatcherTestEnvironment of()
  {
    ...
    result.settings          = tt_CommandSettingsMock  .of();
  ...
  tt_CommandSettingsMock settings;
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/reloader.zs
...
{
  static tt_Reloader of(tt_PlayerSource playerSource, tt_CommandSettings settings)
  {
    let result = new("tt_Reloader");
...
#+begin_src c :tangle build/Typist.pk3/zscript/activatable/sphinx.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/aimer/aimer.zs

// This class is an interface for aiming.
class tt_Aimer abstract
{
  play abstract void aim();
}
#+end_src
...
{
  static tt_AimerMock of() { return new("tt_AimerMock"); }
  ...
  {
    return tt_Satisfaction.of()
      .push(getClassName() .. ": aim", _mock_aim_expected == _mock_aim_called);
  }
...
#+begin_src c :tangle build/Typist.pk3/zscript/aimer/aimers.zs
...
#+begin_src c :noweb-ref test-body
{
  ...
  assertSatisfaction(aimer1.getSatisfaction(), "tt_Aimer: aimer1");
  assertSatisfaction(aimer2.getSatisfaction(), "tt_Aimer: aimer2");
}
#+end_src
...
#+begin_src c :tangle build/Typist.pk3/zscript/event_handler/screen_saver.zs
...
class tt_ScreenSaver : StaticEventHandler
{
  ...
  const REFRESH_RATE = TICRATE / 8;
  const CHAR_RATE    = TICRATE / 4;
  const TIMEOUT      = TICRATE * 60 * 3; // 3 minutes
  const GREEN        = "008800";
...
#+begin_src c :tangle build/Typist.pk3/zscript/firer/firer.zs

// This is an interface for firing at target.
class tt_Firer abstract
{
  play abstract void fire();
}
#+end_src
...
{
  static tt_FirerMock of() { return new("tt_FirerMock"); }
  ...
  {
    return tt_Satisfaction.of()
      .push(getClassName() .. ": fire", _mock_fire_expected == _mock_fire_called);
  }
...
{
  static tt_FirerImpl of(tt_PlayerSource playerSource)
  {
    let result = new("tt_FirerImpl");
...
class tt_GameTweaks play
{
  ...
  static private readonly<PlayerPawn> getDefaultPawn(PlayerPawn pawn)
  {
...
// This class implements tt_InputManager by examining the current and old Typist mode.
// Input is reset when the game mode is changed.
class tt_InputByModeManager : tt_InputManager
{
  static tt_InputByModeManager of(tt_ModeSource modeSource, tt_PlayerInput playerInput)
  {
...
// This class implements tt_KnownTargetSource by reading from targets from
...
}
{
  ...
  env.disabledTargetSource.expect_getTargets(tt_Targets.of());
  env.questionSource.expect_getQuestion(tt_QuestionMock.of(), 2);
...
}
{
  ...
  env.targetSource.expect_getTargets(targets);
  env.disabledTargetSource.expect_getTargets(tt_Targets.of());
  env.questionSource.expect_getQuestion(tt_QuestionMock.of());
  ...
  env.targetSource.expect_getTargets(targets);
  env.disabledTargetSource.expect_getTargets(tt_Targets.of());
  env.questionSource.expect_getQuestion(NULL, 0);
...
class tt_TargetRegistryTestEnvironment
{
  static tt_TargetRegistryTestEnvironment of()
  {
    ...
    result.questionSource       = tt_QuestionSourceMock.of();
...
{
  static tt_QuestionSourceMock of() { return new("tt_QuestionSourceMock"); }
...
{
  static tt_MixedLesson of(tt_Settings settings)
  {
...
class tt_RandomNumberSource : tt_QuestionSource
{
  ...
  override tt_Question getQuestion()
  {
    ...
    {
      int number = Random(tt_su_Ascii.DIGIT_ZERO, tt_su_Ascii.DIGIT_NINE);
...
{
  static tt_SelectedQuestionSource of(tt_Settings settings)
  {
    let result = new("tt_SelectedQuestionSource");
  ...
  override tt_Question getQuestion()
  {
    ...
    if (questionSourceIndex >= _questionSources.size()) return NULL;
...
#+begin_src c :noweb-ref test-body
{
  let tag = "tt_SelectedQuestionSource: checkEmpty";
  let env = tt_SelectedQuestionSourceTestEnvironment.of();

  env.settings.expect_getQuestionSourceIndex(0);

  let question = env.selectedQuestionSource.getQuestion();

  it(tag .. ": no sources -> NULL question", AssertNull(question));
  assertSatisfaction(env.getSatisfaction(), tag);
}
{
  let tag = "tt_SelectedQuestionSource: checkTwo";
  let env = tt_SelectedQuestionSourceTestEnvironment.of();

  let questionSource1 = tt_QuestionSourceMock.of();
  let questionSource2 = tt_QuestionSourceMock.of();

  env.selectedQuestionSource.add(questionSource1);
  env.selectedQuestionSource.add(questionSource2);

  let mockQuestion = tt_QuestionMock.of();
  questionSource2.expect_getQuestion(mockQuestion);
  env.settings   .expect_getQuestionSourceIndex(1);

  let question = env.selectedQuestionSource.getQuestion();

  assertSatisfaction(questionSource2.getSatisfaction(), tag);
...
class tt_SelectedQuestionSourceTestEnvironment
{
  static tt_SelectedQuestionSourceTestEnvironment of()
  {
    let result = new("tt_SelectedQuestionSourceTestEnvironment");
    result.settings               = tt_SettingsMock.of();
    result.selectedQuestionSource = tt_SelectedQuestionSource.of(result.settings);
    return result;
  }
  ...
  tt_SelectedQuestionSource selectedQuestionSource;
}
#+end_src
...
#+begin_src c :tangle build/Typist.pk3/zscript/menu/list_menu_note.zs
...
#+begin_src c :tangle build/Typist.pk3/zscript/menu/menu_injector.zs
...
#+begin_src c :noweb-ref test-body
{
  ...
  for (int playerNumber = 0; playerNumber < MAXPLAYERS; ++playerNumber)
  {
    ...
    it(string.format("tt_PlayerSourceImpl: player info (%d) must be not NULL",
                     playerNumber),
       Assert(info != NULL));
  }
}
{
  ...
  it(string.format("tt_PlayerSourceImpl: must get main player (%d) actor",
                   consolePlayer),
     AssertNotNull(pawn));
}
{
  ...
  for (int i = 1; i < MAXPLAYERS; ++i)
  {
    ...
    it(string.format("tt_PlayerSourceImpl: other player (%d) must be null",
                     playerNumber),
       AssertNull(pawn));
  }
}
...
{
  abstract string getForwardDashCommand();

  abstract string getBackwardDashCommand();

  abstract string getLeftDashCommand();

  abstract string getRightDashCommand();

  abstract string getReloadCommand();
...
class tt_CommandSettingsMock : tt_CommandSettings
{
  ...
  override string getForwardDashCommand()
  {
  ...
  override string getBackwardDashCommand()
  {
  ...
  override string getLeftDashCommand()
  {
  ...
  override string getRightDashCommand()
  {
  ...
  override string getReloadCommand()
  {
  ...
  override string getCommandPrefix()
  {
  ...
  tt_Satisfaction getSatisfaction() const
  {
    let aName = getClassName();
    return tt_Satisfaction.of()
      .push(aName .. ": getForwardDashCommand",
            _mock_getForwardDashCommand_expected ==
            _mock_getForwardDashCommand_called)
      .push(aName .. ": getBackwardDashCommand",
            _mock_getBackwardDashCommand_expected ==
            _mock_getBackwardDashCommand_called)
      .push(aName .. ": getLeftDashCommand",
            _mock_getLeftDashCommand_expected == _mock_getLeftDashCommand_called)
      .push(aName .. ": getRightDashCommand",
            _mock_getRightDashCommand_expected == _mock_getRightDashCommand_called)
      .push(aName .. ": getReloadCommand",
            _mock_getReloadCommand_expected == _mock_getReloadCommand_called)
      .push(aName .. ": getCommandPrefix",
            _mock_getCommandPrefix_expected == _mock_getCommandPrefix_called);
  }
...
class tt_Settings abstract
{
  ...
  abstract int getQuestionSourceIndex();
...
class tt_SettingsMock : tt_Settings
{
  ...
  override int getQuestionSourceIndex()
  {
  ...
  tt_Satisfaction getSatisfaction()
  {
    return tt_Satisfaction.of()
      ...
      .push(getClassName() .. "getQuestionSourceIndex",
            _mock_getQuestionSourceIndex_expected ==
            _mock_getQuestionSourceIndex_called)
...
#+begin_src c :noweb-ref test-body
{
  ...
  assertSatisfaction(playerSource.getSatisfaction(), tag);
  it(tag .. ": scale is more than zero",
     AssertEval(scale, ">", 0));
}
{
...
{
  let tag = "tt_SettingsImpl: testGetQuestionSource";
  let playerSource = tt_PlayerSourceMock.of();
  let settings     = tt_SettingsImpl.of(playerSource);
  ...
  it(tag .. ": question source index is reasonable",
     AssertEval(questionSourceIndex, ">=", 0));
  assertSatisfaction(playerSource.getSatisfaction(), tag);
}
#+end_src
...
class tt_ConditionalView : tt_View
{
  ...
  private ui bool isInMenu()
  {
    return menuActive;
  }

  private ui bool isLevelClear()
  {
...
class tt_Drawing
{
  static int getWidthForTarget(String question, string answer, tt_Settings settings)
  {
    Font fnt         = NewSmallFont;
...
#+begin_src c :tangle build/Typist.pk3/zscript/world_changer/answer_resetter.zs
...
class tt_EnemySpeedController : tt_WorldChanger
{
  ...
  private play bool isSlowedDown(Actor a)
  {
    return (a.CountInv("tt_VelocityStorage") != 0);
  }

  private play void slowDown(Actor a)
  {
...
#+begin_src c :tangle build/Typist.pk3/zscript/world_changer/gunner.zs
...
{
  static tt_Gunner of(tt_OriginSource originSource, tt_Aimer aimer, tt_Firer firer)
  {
...
}
{
  ...
  env.aimer.expect_aim();
  env.firer.expect_fire();
...
class tt_GunnerTestEnvironment
{
  static tt_GunnerTestEnvironment of()
  {
    ...
    result.originSource = tt_OriginSourceMock.of();
    result.aimer        = tt_AimerMock.of();
    result.firer        = tt_FirerMock.of();

    result.gunner = tt_Gunner.of(result.originSource, result.aimer, result.firer);
    return result;
  }
  ...
  tt_Satisfaction getSatisfaction() const
  {
    return originSource.getSatisfaction()
      .add(aimer.getSatisfaction())
      .add(firer.getSatisfaction());
  }
...
#+begin_src c :tangle build/Typist.pk3/zscript/world_changer/match_watcher.zs
...
class tt_ProjectileSpeedController : tt_WorldChanger
{
  ...
  private play void restoreVelocity(Actor a)
  {
...
{
  static tt_WorldChangers of(Array<tt_WorldChanger> worldChangers)
  {
...
// TODO: replace with native cvar resetting
...
TT_DISPLAY_OPTIONS_TITLE_OUT  = "Display Options";
TT_DISPLAY_OPTIONS_TITLE_IN   = "Typist.pk3 Display Options";
TT_TARGET_SCALE               = "Target text scale";
TT_SHOW_NOTES                 = "Notes in Main Menu";
TT_SCREEN_SAVER               = "Screensaver";

TT_GAMEPLAY_OPTIONS_TITLE_OUT = "Gameplay Options";
TT_GAMEPLAY_OPTIONS_TITLE_IN  = "Typist.pk3 Gameplay Options";
...
TT_COMMANDS_TITLE_OUT  = "Command Options";
TT_COMMANDS_TITLE_IN   = "Typist.pk3 Command Options";

TT_COMMAND_PREFIX      = "Command prefix";
TT_COMMAND_PREFIX_NOTE = "It's inadvisable to select a prefix that may occur in a lesson.";

TT_FORWARD_DASH  = "Dash forward";
TT_BACKWARD_DASH = "Dash backward";
TT_RIGHT_DASH    = "Dash right";
TT_LEFT_DASH     = "Dash left";
TT_RELOAD        = "Reload";

TT_RESET_COMMAND = "Reset to default";
#+end_src
