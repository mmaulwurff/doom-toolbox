...
LazyPoints module version: 0.7.0
LazyPoints is a part of DoomToolbox.
...
nosave string NAMESPACE_lp_score = "";
user   bool   NAMESPACE_lp_show  = true;
server string NAMESPACE_lp_parameters_class = "NAMESPACE_lp_Parameters";
#+end_src

#+name: mapinfo
#+begin_src c

GameInfo { EventHandlers = "NAMESPACE_lp_Dispatcher", "NAMESPACE_lp_StaticView" }
#+end_src

#+name: menudef
#+begin_src c

OptionMenu NAMESPACE_lp_TopMenu
{
  class NAMESPACE_lp_Top
  Title "Top Points"
}
#+end_src

#+begin_src c :tangle ../build/LazyPoints/LazyPoints.zs :exports none
...
class NAMESPACE_lp_Parameters
{
  // This font is used for drawing score points and info on bonuses.
  virtual Font getFont() const { return NAMESPACE_Fonts.findBig(); }
...
class NAMESPACE_lp_Dispatcher : EventHandler
{
  override void onRegister() { _spawner = new ("NAMESPACE_lp_Spawner").init(); }

  override void playerEntered(PlayerEvent event)
  {
    if (_parameters == NULL)
      _parameters = NAMESPACE_lp_Parameters(new(NAMESPACE_lp_parameters_class));

    if (_mapScore == NULL)
      _mapScore = new ("NAMESPACE_lp_MapScore").init();

    _playerScores[event.playerNumber] =
      new("NAMESPACE_lp_PlayerScore").init(event.playerNumber, _parameters);
  }
  ...
  private NAMESPACE_lp_Spawner _spawner;
  private NAMESPACE_lp_PlayerScore _playerScores[MAXPLAYERS];
  private NAMESPACE_lp_Parameters _parameters;
  private NAMESPACE_lp_MapScore _mapScore;
}
#+end_src
...
class NAMESPACE_lp_BonusView
{
  NAMESPACE_lp_BonusView init(NAMESPACE_lp_TimerBonus timerBonus,
                           NAMESPACE_lp_HealthBonus healthBonus,
                           NAMESPACE_lp_Parameters parameters)
  {
  ...
  mixin NAMESPACE_lp_FontUser;

  private NAMESPACE_lp_TimerBonus _timerBonus;
  private NAMESPACE_lp_HealthBonus _healthBonus;
}
#+end_src
...
class NAMESPACE_lp_Counter
{
  NAMESPACE_lp_Counter init(int playerNumber,
                         NAMESPACE_lp_TimerBonus timerBonus,
                         NAMESPACE_lp_HealthBonus healthBonus,
                         NAMESPACE_lp_Parameters parameters)
  {
...
mixin class NAMESPACE_lp_FontUser
{
...
class NAMESPACE_lp_HealthBonus
{
  NAMESPACE_lp_HealthBonus init(int playerNumber)
  {
    _player = players[playerNumber];
...
class NAMESPACE_lp_MapScore
{
  NAMESPACE_lp_MapScore init()
  {
  ...
  void save()
  {
    ...
    NAMESPACE_lp_ScoreStorage.saveScore(checksum, score);
  }
...
class NAMESPACE_lp_MapScoreItem : ScoreItem
{
  NAMESPACE_lp_MapScoreItem init(int n)
  {
    amount = n;
...
class OptionMenuScoreItem : OptionMenuItem
{
  ...
  {
    _name = s;
    NAMESPACE_lp_ScoreStorage.rename(Level.GetChecksum(), _index, _name);
    return true;
  }
...
class NAMESPACE_lp_PlayerScore
{
  NAMESPACE_lp_PlayerScore init(int playerNumber, NAMESPACE_lp_Parameters parameters)
  {
    _playerNumber = playerNumber;
    _parameters   = parameters;

    _timer = new ("NAMESPACE_lp_Timer").init(TICRATE * parameters.getBonusCountdown());
    _timerBonus  = new ("NAMESPACE_lp_TimerBonus").init(_timer);
    _healthBonus = new ("NAMESPACE_lp_HealthBonus").init(playerNumber);
    _counter = new("NAMESPACE_lp_Counter").init(playerNumber,
                                             _timerBonus,
                                             _healthBonus,
                                             parameters);
    ...
    {
      _view      = new ("NAMESPACE_lp_View").init(parameters);
      _timerView = new ("NAMESPACE_lp_TimerView").init(_timer);
      _bonusView = new ("NAMESPACE_lp_BonusView").init(_timerBonus,
                                                    _healthBonus,
                                                    parameters);
    }
  ...
  {
    if (_isVisible == NULL)
      _isVisible = CVar.GetCVar("NAMESPACE_lp_show", players[_playerNumber]);
  ...
  private NAMESPACE_lp_Timer _timer;
  private NAMESPACE_lp_TimerBonus _timerBonus;
  private NAMESPACE_lp_HealthBonus _healthBonus;
  private NAMESPACE_lp_Counter _counter;

  private NAMESPACE_lp_View _view;
  private NAMESPACE_lp_TimerView _timerView;
  private NAMESPACE_lp_BonusView _bonusView;
...
class NAMESPACE_lp_ScoreStorage
{
  ...
  const STORAGE_CVAR_NAME = "NAMESPACE_lp_score";
}
#+end_src
...
class NAMESPACE_lp_Spawner
{
  NAMESPACE_lp_Spawner init() { return self; }

  play void spawnScoreFor(Actor thing)
  {
    if (thing && thing.bCountItem)
    {
      NAMESPACE_lp_MapScoreItem(Actor.Spawn("NAMESPACE_lp_MapScoreItem", thing.pos))
          .init(5);
    }
...
class NAMESPACE_lp_StaticView : StaticEventHandler
{
  override void onRegister() { _topHintView = new ("NAMESPACE_lp_TopHintView").init(); }

  override void uiTick() { _topHintView.show(); }

  private NAMESPACE_lp_TopHintView _topHintView;
}
#+end_src
...
class NAMESPACE_lp_Timer
{
  // Initializes an object with count - number of ticks to count.
  NAMESPACE_lp_Timer init(int count)
  {
    _count        = count;
    _currentCount = 0;
...
// kill is registered in limited time frame (provided by NAMESPACE_lp_Timer).
class NAMESPACE_lp_TimerBonus
{
  NAMESPACE_lp_TimerBonus init(NAMESPACE_lp_Timer timer)
  {
    _bonus = 0;
    _timer = timer;
...
class NAMESPACE_lp_TimerView
{
  NAMESPACE_lp_TimerView init(NAMESPACE_lp_Timer timer)
  {
    _timer = timer;
...
class NAMESPACE_lp_Top : OptionMenu
{
  override void Init(Menu parent, OptionMenuDescriptor desc)
  {
    ...
    NAMESPACE_lp_ScoreStorage.loadScores(checksum, scores, isLatest, names);

    int maxLength = 0;
    for (int i = 0; i < NAMESPACE_lp_ScoreStorage.N_TOP; ++i)
    {
...
class NAMESPACE_lp_TopHintView
{

  NAMESPACE_lp_TopHintView init()
  {
    _showed = false;
  ...
  void show()
  {
    ...
    int key1;
    int key2;
    [key1, key2] = Bindings.GetKEysForCommand("NAMESPACE_lp_top");
...
class NAMESPACE_lp_View
{
  NAMESPACE_lp_View init(NAMESPACE_lp_Parameters parameters)
  {
...
Alias NAMESPACE_lp_top "OpenMenu NAMESPACE_lp_TopMenu"
AddKeySection "LazyPoints" "NAMESPACE_lp_keys"
AddMenuKey    "Open Score" "NAMESPACE_lp_top"
#+end_src
...
#include "zscript/NAMESPACE_Fonts.zs"
#+end_src
