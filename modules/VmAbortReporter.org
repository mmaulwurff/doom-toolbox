...
#+name: meta
#+begin_src js
{ "depends": { "StringUtils": "NAMESPACE_" } }
#+end_src
...
VmAbortReporter module version: 1.0.1

VmAbortReporter is a part of DoomToolbox.
...
#+begin_src c :tangle ../build/VmAbortReporter/VmAbortReporter.zs :exports none
...
{
  const REPORT_ENABLED_CVAR_NAME = "NAMESPACE_vm_abort_report_enabled";

  override void playerSpawned(PlayerEvent event)
  {
    Cvar enabledCvar = Cvar.getCvar(REPORT_ENABLED_CVAR_NAME, players[consolePlayer]);
    if (enabledCvar == NULL)
      Console.printf("ERROR: %s Cvar is not defined.", REPORT_ENABLED_CVAR_NAME);
...
#+begin_src c :tangle ../build/VmAbortReporter/zscript.zs
