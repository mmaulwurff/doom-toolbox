...
LispOnZscript module version: v0.0.1
LispOnZscript is a part of DoomToolbox: https://github.com/mmaulwurff/doom-toolbox/

LispOnZscript is a toy Lisp-like language written on top of ZScript for UZDoom.

LispOnZscript is an experiment, a programming exercise, and isn't suitable for
any practical purpose. I'll be surprised if it finds any practical purpose. See
tests to see what expressions it can evaluate at the moment, also it contains
~tl_Eval.eval~ usage example. Search for ~TODO~ in this file to see what remains
to be done to make LispOnZscript more usable.
...
#+begin_src c :tangle ../build/LispOnZscript/LispOnZscript.zs
...
class tl_Test
{
  static void run()
  {
    ...
    cases.insert("1", "1");
    cases.insert("(+ 1 2)", "3");
    cases.insert("(+ (* 2 3) 4)", "10");
    cases.insert("(quote 2)", "2");
    cases.insert("(quote (+ 1 2))", "(+ 1 2)");
    cases.insert("(atom? (+ 1 2))", "True");
    cases.insert("(atom? (q (+ 1 2)))", "False");
    cases.insert("(eq? 3 3)", "True");
    cases.insert("(car (quote (1 2 3)))", "1");
    cases.insert("(cdr (quote (1 2 3)))", "(2 3)");
    cases.insert("(cons 1 (quote (2 3)))", "(1 2 3)");
    cases.insert("(cond ((< 1 0) 2) ((< 0 1) 3))", "3");
    cases.insert("(null? (quote ()))", "True");
    cases.insert("(null? 2)", "False");
    cases.insert("(if True 1 2)", "1");
    cases.insert("(if False 1 2)", "2");
    cases.insert("(begin 1 2 3)", "3");
    cases.insert("(begin (set! abc 3) abc)", "3");
    cases.insert("(begin (define abc 3) abc)", "3");
    cases.insert("(begin (define square (lambda (x) (* x x))) (square 3))", "9");
    cases.insert("(begin (define cube (lambda (x) (* x (* x x)))) (cube 2))", "8");
    cases.insert("\"hello world\"", "\"hello world\"");
    cases.insert("(+ \"hello\" \" world\")", "\"hello world\"");
    cases.insert("\"️♥\"", "\"️♥\"");

    cases.insert("",    "Error: unexpected EOF while reading.");
    cases.insert("()",  "Error: unexpected empty list.");
    cases.insert("(",   "Error: unmatched \"(\".");
    cases.insert(")",   "Error: unexpected \")\".");
    cases.insert("(2)", "Error: \"2\" isn't a function.");
    cases.insert("(+ 1 2 3)",
                 "Error: TODO: add support for different number of parameters.");

    cases.insert("player", "DoomPlayer");
    cases.insert("(getCharacterName player)", "\"DoomPlayer\"");
    cases.insert("(playerNumber player)", "0");
    cases.insert("(isFrozen player)", "False");
    cases.insert("(isFriend player player)", "True");

    int successCount = 0;

    foreach (input, expected: cases)
    {
      Console.printf("Testing \"%s\", expecting \"%s\"", input, expected);

      tl_Expression inputExpression = tl_Parser.parse(input);
      if (inputExpression == NULL)
      {
        Console.printf("ERROR: parse result is NULL");
        continue;
      }
      ...
      {
        Console.printf("ERROR: eval result is NULL");
        continue;
      }
      ...
      if (result == expected)
        ...
        Console.printf("ERROR: expected: \"%s\", got \"%s\"", expected, result);
    }

    string result = (successCount == cases.countUsed()) ? "Success" : "Fail";
    Console.printf("%s: %d succeeded of %d", result, successCount, cases.countUsed());
  }
}
#+end_src

#+begin_src text :tangle ../build/LispOnZscriptTest/commands.txt
wait 2; map map01; wait 2; netevent lisp-test; wait 2; quit
#+end_src

#+begin_src c :tangle ../build/LispOnZscript/mapinfo.txt

// <<license>>

GameInfo { EventHandlers = "tl_TestEventHandler" }
#+end_src

#+begin_src c :tangle ../build/LispOnZscript/zscript.zs

#include "LispOnZscript.zs"
#include "zscript/tl_StringUtils.zs"

class tl_TestEventHandler : StaticEventHandler
{
  override void networkProcess(ConsoleEvent event)
  {
    if (event.name == "lisp-test") tl_Test.run();
  }
}
#+end_src
...
TODO: change namespace to ~NAMESPACE_~.

#+begin_src c :noweb-ref source
...
class tl_Parser
{
  ...
  private static void tokenize(string input, out Array<string> tokens)
  {
    ...
    {
      int character;
      [character, i] = input.getNextCodePoint(i);
      int kind = getCharacterKind(character);
      bool wasInString = isInString;
      isInString ^= kind == CharacterDoubleQuote;

      if (wasInString || kind == CharacterNormal)
      {
        currentToken.appendCharacter(character);

        if (kind == CharacterDoubleQuote)
          currentToken = commit(currentToken, tokens);
      ...
      else if (kind == CharacterDelimiter)
        commit(string.format("%c", character), tokens);
    }

    if (currentToken.length() != 0)
      commit(currentToken, tokens);
  }

  private static string commit(string token, out Array<string> tokens)
  {
    tokens.push(token);
    return "";
  }
  ...
  enum _
  {
    CharacterSpace,
    CharacterDelimiter,
    CharacterDoubleQuote,
    CharacterNormal,
  }

  private static int getCharacterKind(int character)
  {
    if (isSpace(character)) return CharacterSpace;
    if (isDelimiter(character)) return CharacterDelimiter;
    if (character == 0x22) return CharacterDoubleQuote;
    return CharacterNormal;
  }

  private static bool isDelimiter(int character)
  {
    return character == tl_Ascii.LEFT_PARENTHESIS
      || character == tl_Ascii.RIGHT_PARENTHESIS;
  }

  private static bool isSpace(int character)
  {
    return character == tl_Ascii.SPACE
      || character == tl_Ascii.CHARACTER_TABULATION
      || character == tl_Ascii.CARRIAGE_RETURN_CR
      || character == tl_Ascii.LINE_FEED_LF;
  }
  ...
  private static tl_Expression atom(string token)
  {
    if (token.getNextCodePoint(0) == 0x22)
      return tl_String.from(token.mid(1, token.length() - 2));
  ...
  static tl_Expression eval(tl_Expression expression, tl_Env env)
  {
    ...
    if (symbol != NULL)
    {
      ...
      if (symbol.name == "car")
      {
        ...
        if (resultList == NULL) return error("car argument isn't a list");
      ...
      if (symbol.name == "cdr")
      {
        ...
        if (resultList == NULL) return error("cdr argument isn't a list");
    ...
    if (procFunction == NULL)
      return error(string.format("\"%s\" isn't a function", proc.toString()));
...
class tl_AutoFunction : tl_Function
{
  ...
  override tl_Expression execute(Array<tl_Expression> exps)
  {
    ...
    if (objectExpression == NULL)
      return error(string.format("cannot call \"%s\": \"%s\" isn't an object",
                                 name,
                                 exps[0].toString()));
...
class tl_Globals
{
  static tl_Expression add(tl_Expression lhs, tl_Expression rhs)
  {
    ...
    let lhsString = tl_String(lhs);
    let rhsString = tl_String(rhs);
    if (lhsString != NULL && rhsString != NULL)
      return tl_String.from(lhsString.value .. rhsString.value);
