...
StringUtils module version: 1.5.0

StringUtils is a part of DoomToolbox.
...
SPDX-FileCopyrightText: © 2024 Alexander Kromm (m8f) <mmaulwurff@gmail.com>
SPDX-License-Identifier: BSD-3-Clause
#+end_src

* Source

#+begin_src c :tangle ../build/StringUtils/StringUtils.zs :exports none
...
This class acts as a namespace for static functions.
...
Splits a UTF-8 text by words. Characters considered word delimiters: ASCII control
characters, space, comma (','), period ('.'). Delimiters aren't included in the
output.
...
static clearscope bool beginsWith(string aString,
                                  string subString,
                                  bool caseSensitive = false)
{
  if (!caseSensitive)
  {
    ...
    subString = subString.makeLower();
  }
  return aString.left(subString.length()) == subString;
}
#+end_src
#+begin_src c :noweb-ref TestsBody
{
  it ("beginsWith: yes", Assert(su.beginsWith("begins with", "BEGINS")));
  it ("beginsWith: no", Assert(!su.beginsWith("begins with", "ends")));
  it ("beginsWith: case: yes", Assert(su.beginsWith("begins with", "beg", true)));
  it ("beginsWith: case: no", Assert(!su.beginsWith("begins with", "BEG", true)));
  it ("beginsWith: unicode: yes", Assert(su.beginsWith("мышь", "Мы")));
  it ("beginsWith: unicode: no", Assert(!su.beginsWith("мышь", "они")));
  it ("beginsWith: empty string", Assert(!su.beginsWith("", "?")));
  it ("beginsWith: empty substring", Assert(su.beginsWith("?", "")));
  it ("beginsWith: empty both", Assert(su.beginsWith("", "")));
}
#+end_src
...
#+begin_src c :noweb-ref TestsBody
{
  ...
  it("ASCII ',' isn't a control character",
