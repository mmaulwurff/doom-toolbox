#+property: header-args :comments no :mkdirp yes :noweb yes :results none

#+name: license
#+begin_src text :exports none
SPDX-FileCopyrightText: © 2022 Alexander Kromm <mmaulwurff@gmail.com>
SPDX-License-Identifier: GPL-3.0-only
#+end_src

#+title: SoundToScreen
# #+version: v1.0.0

* About

SoundToScreen shows where sounds come from. It may be useful for people who play
without sound or people with hearing impairment.

** Features

TODO: try to make savefile-compatible with SoundToScreen v1.0.0.
- Compatible with saves saved without SoundToScreen (not compatible with saves saved
  with SoundToScreen < 2.0.0)

** Implementation Notes

- SoundToScreen won't work if UZDoom is launched with ~-nosound~.
- doors and elevators that only wait to move are detected as producing
  sound. There is no way to detect this properly at the moment.

** Inspiration

- [[https://forum.zdoom.org/viewtopic.php?f=4&t=71349][Toby Accessibility Mod]] by Alando1.
- [[https://youtu.be/4NGe4dzlukc][Making Games Better for the Deaf and Hard of Hearing]] by Game Maker's Toolkit.

* License

[[file:LICENSES/GPL-3.0-only.txt][GPL-3.0-only]]

#+begin_src c :tangle build/SoundToScreen/cvarinfo.txt :exports none
// <<license>>
#+end_src
#+begin_src c :tangle build/SoundToScreen/language.txt :exports none
// <<license>>
#+end_src
#+begin_src c :tangle build/SoundToScreen/mapinfo.txt :exports none
// <<license>>
#+end_src
#+begin_src c :tangle build/SoundToScreen/menudef.txt :exports none
// <<license>>
#+end_src
#+begin_src c :tangle build/SoundToScreen/zscript.zs :exports none
// <<license>>
#+end_src

* language

#+begin_src c :tangle build/SoundToScreen/language.txt

[enu default]

STS_MENU_NAME  = "\cj⚞\c- SoundToScreen Options";
STS_MENU_TITLE = "SoundToScreen";

STS_ON_AUTOMAP = "On automap";

STS_COLOR_NOISE    = "Noise color";
STS_COLOR_GEOMETRY = "Level geometry change color";
STS_COLOR_DANGER   = "Danger color";

STS_MAX_DISTANCE  = "Maximum distance to sound";
#+end_src

* menudef

// TODO: use PlainTranslator.
#+begin_src c :tangle build/SoundToScreen/menudef.txt

AddOptionMenu OptionsMenu       { Submenu "$STS_MENU_NAME", sts_Menu }
AddOptionMenu OptionsMenuSimple { Submenu "$STS_MENU_NAME", sts_Menu }

OptionMenu sts_Menu
{
  Title "$STS_MENU_TITLE"

  Slider "Horizontal position", sts_x_position, 0.0, 1.0, 0.01, 2
  Slider "Vertical position",   sts_y_position, 0.0, 1.0, 0.01, 2
  Slider "Width",               sts_x_radius, 20, 50, 1
  Slider "Height",              sts_y_radius, 20, 50, 1
  Slider "Dot size",            sts_dot_size, 1, 9, 2

  Option "$STS_ON_AUTOMAP", sts_on_automap, OnOff

  StaticText ""
  StaticText "Colors"
  ColorPicker "Base", sts_color_base
  ColorPicker "$STS_COLOR_NOISE",    sts_color_noise
  ColorPicker "$STS_COLOR_GEOMETRY", sts_color_geometry
  ColorPicker "$STS_COLOR_DANGER",   sts_color_danger

  StaticText ""
  Slider "$STS_MAX_DISTANCE", sts_max_distance2, 1, 500, 1, 0
}
#+end_src

* Cvars

#+begin_src c :tangle build/SoundToScreen/cvarinfo.txt

user float sts_x_position = 0.5;
user float sts_y_position = 0.7;
user float sts_x_radius = 3;
user float sts_y_radius = 3;
user int   sts_dot_size = 3;

user bool  sts_on_automap = false;
user int   sts_max_distance2 = 50;

user color sts_color_base     = "000000"; // black
user color sts_color_noise    = "FFFFFF"; // white
user color sts_color_geometry = "00FF00"; // green
user color sts_color_danger   = "FF0000"; // red
#+end_src

* Event Handler

#+begin_src c :tangle build/SoundToScreen/mapinfo.txt

GameInfo { EventHandlers = "sts_EventHandler" }
#+end_src

#+begin_src c :tangle build/SoundToScreen/zscript.zs

version 4.13.3

class sts_EventHandler : StaticEventHandler
{
  // Collects sounds by searching for actors that make sounds and doors/elevators.
  override void worldTick()
  {
    if (!mIsInitialized && !(level.mapName ~== "TITLEMAP")) initialize();

    if (players[consolePlayer].mo == NULL) return;
    if (automapActive && !mOnAutomapCvar.getInt()) return;

    mIterator.reinit();

    for (int type = Noise; type < SoundTypesCount; ++type)
      mColors[type] = mColorCvars[type].getString();

    for (int type = Noise; type < SoundTypesCount; ++type)
    {
      for (int i = 0; i < DOTS_COUNT; ++i)
        mDots[type][i] = 0;
    }

    int maxDistance = mMaxDistanceCvar.getInt();

    let player = players[consolePlayer].mo;
    Thinker aThinker;
    while (aThinker = mIterator.next())
    {
      let anActor = Actor(aThinker);
      if (anActor != NULL)
      {
        if (anActor == player) continue;
        if (anActor is "Inventory" && Inventory(anActor).owner != NULL) continue;
        if (!anActor.isActorPlayingSound(CHAN_AUTO)) continue;
        double distance = anActor.distance3D(player) / DISTANCE_UNIT;
        if (distance > maxDistance) continue;

        let type = ((anActor.bIsMonster && !anActor.bFriendly && anActor.health > 0)
                    || (anActor.bMissile && anActor.damage > 0)) ? Danger : Noise;

        let position = calculateActorScreenPosition(anActor);
        mDots[type][position] += min(1.0, 1.0 / max(distance * distance / 100, 1.0));

        continue;
      }

      let aMover = Mover(aThinker);
      if (aMover != NULL)
      {
        Sector aSector = aMover.getSector();
        if (aSector.flags & Sector.SECF_SilentMove) continue;

        // Important order: ask if moving before remembering.
        bool isMoving = isMoving(aSector);
        remember(aSector);
        if (!isMoving) continue;

        vector3 playerRelative = player.posRelative(aSector);
        vector2 xy = aSector.centerSpot;
        double z = aMover is "MovingCeiling"
          ? aSector.centerCeiling()
          : aSector.centerFloor();
        vector3 soundSource = (xy.x, xy.y, z);
        vector3 diff = level.vec3Diff(soundSource, playerRelative);
        double distance = diff.length() / DISTANCE_UNIT;
        if (distance > maxDistance) continue;

        let position = calculateSectorScreenPosition(aSector);
        mDots[Geometry][position] += min(1.0, 1.0 / max(distance * distance / 100, 1.0));
      }
    }
  }

  override void renderOverlay(RenderEvent event)
  {
    if (!mIsInitialized) return;
    if (players[consolePlayer].mo == NULL) return;

    vector2 center = (mXPositionCvar.getFloat() * Screen.getWidth(),
                      mYPositionCvar.getFloat() * Screen.getHeight());
    double radiusX = mXRadiusCvar.getFloat();
    double radiusY = mYRadiusCvar.getFloat();
    int    dotSize = mDotSizeCvar.getInt();
    Color  baseColor = mBaseColorCvar.getString();

    for (int i = 0; i < DOTS_COUNT; ++i)
      drawDot(center + makePosition(i, 0.0, radiusX, radiusY), baseColor, dotSize);

    for (int type = Noise; type < SoundTypesCount; ++type)
    {
      for (int i = 0; i < DOTS_COUNT; ++i)
      {
        double diff = mDotsInterpolated[type][i] - mDots[type][i];
        mDotsInterpolated[type][i] -= diff * ((diff > 0) ? 0.2 : 0.4);
      }
    }

    // Fake sound level changes.
    if (level.time % 3 == 0)
    {
      for (int type = Noise; type < SoundTypesCount; ++type)
      {
        for (int i = 0; i < DOTS_COUNT; ++i)
          mDotsVariance[type][i] = frandom[SoundToScreen](-0.05, 0.05);
      }
    }

    for (int type = Noise; type < SoundTypesCount; ++type)
    {
      for (int i = 0; i < DOTS_COUNT; ++i)
      {
        double sum = 0;
        for (int j = -2; j <= 2; ++j)
        {
          int index = (i + j + DOTS_COUNT) % DOTS_COUNT;
          sum += mDotsInterpolated[type][index] * mBlurWeights[2 + j];

          if (mDotsInterpolated[type][index] > 0.05)
            sum += mDotsVariance[type][index];
        }
        mDotsBlurred[type][i] = min(1.0, sum);
      }
    }

    for (int type = Noise; type < SoundTypesCount; ++type)
    {
      for (int i = 0; i < DOTS_COUNT; ++i)
      {
        double dotLevel = mDotsBlurred[type][i];
        if (dotLevel > 0.05)
        {
          drawDot(center + makePosition(i, dotLevel, radiusX, radiusY),
                  mColors[type],
                  dotSize);
        }
      }
    }
  }

  private ui vector2 makePosition(int i, double dotLevel, double radiusX, double radiusY)
  {
    double angle = i * (360.0 / DOTS_COUNT);
    return ((1 + dotLevel) * radiusX * sin(angle),
           -(1 + dotLevel) * radiusY * cos(angle));
  }

  private ui void drawDot(vector2 position, Color aColor, int dotSize)
  {
    int halfSize = dotSize / 2;
    Screen.dim(aColor, 1.0,
               int(round(position.x)) - halfSize,
               int(round(position.y)) - halfSize,
               dotSize,
               dotSize);
  }

  private void initialize()
  {
    mIsInitialized = true;
    mIterator = ThinkerIterator.create("Thinker");

    PlayerInfo player = players[consolePlayer];
    mXPositionCvar    = Cvar.getCvar("sts_x_position", player);
    mYPositionCvar    = Cvar.getCvar("sts_y_position", player);
    mXRadiusCvar      = Cvar.getCvar("sts_x_radius", player);
    mYRadiusCvar      = Cvar.getCvar("sts_y_radius", player);
    mDotSizeCvar      = Cvar.getCvar("sts_dot_size", player);

    mOnAutomapCvar    = Cvar.getCvar("sts_on_automap", player);
    mMaxDistanceCvar  = Cvar.getCvar("sts_max_distance2", player);

    mBaseColorCvar        = Cvar.getCvar("sts_color_base", player);
    mColorCvars[Noise]    = Cvar.getCvar("sts_color_noise", player);
    mColorCvars[Geometry] = Cvar.getCvar("sts_color_geometry", player);
    mColorCvars[Danger]   = Cvar.getCvar("sts_color_danger", player);

    // Gaussian blur, sigma 1.
    mBlurWeights[0] = 0.0613595978134402  / 0.38774041331389975;
    mBlurWeights[1] = 0.24477019552960988 / 0.38774041331389975;
    mBlurWeights[2] = 1.0;
    mBlurWeights[3] = mBlurWeights[1];
    mBlurWeights[4] = mBlurWeights[0];
  }

  private static int calculateActorScreenPosition(Actor target)
  {
    PlayerInfo player = players[consolePlayer];
    double angleToTarget = (player.mo.angle - player.mo.angleTo(target)) % 360.0;
    return int(round(angleToTarget / (360.0 / DOTS_COUNT))) % DOTS_COUNT;
  }

  private static int calculateSectorScreenPosition(sector aSector)
  {
    PlayerInfo player = players[consolePlayer];
    vector3 playerRelative = player.mo.posRelative(aSector);
    vector2 diff = aSector.centerSpot - playerRelative.xy;
    double angleToTarget = (player.mo.angle - atan2(diff.y, diff.x)) % 360.0;
    return int(round(angleToTarget / (360.0 / DOTS_COUNT))) % DOTS_COUNT;
  }

  // No way to easily query if a sector is moving, hence this workaround.
  private bool isMoving(Sector aSector)
  {
    let memory = sts_SectorMemory(mSectorMemories.getIfExists(aSector.index()));
    return memory != NULL
        && memory.time == level.time - 1
        && (memory.floorHeight != aSector.centerFloor()
            || memory.ceilingHeight != aSector.centerCeiling());
  }

  private void remember(Sector aSector)
  {
    let memory = sts_SectorMemory(mSectorMemories.getIfExists(aSector.index()));
    if (memory == NULL)
    {
      memory = new("sts_SectorMemory");
      mSectorMemories.insert(aSector.index(), memory);
    }

    memory.time = level.time;
    memory.floorHeight   = aSector.centerFloor();
    memory.ceilingHeight = aSector.centerCeiling();
  }

  enum SoundType {Noise, Geometry, Danger, SoundTypesCount}
  const SCREEN_CENTER = 0.5;
  const DISTANCE_UNIT = 32;
  const DOTS_COUNT = 32; // Should be divisible by 4 for clear base directions.
  private double mDots[SoundTypesCount][DOTS_COUNT];
  private ui double mDotsInterpolated[SoundTypesCount][DOTS_COUNT];
  private ui double mDotsBlurred[SoundTypesCount][DOTS_COUNT];
  private ui double mDotsVariance[SoundTypesCount][DOTS_COUNT];
  private double mBlurWeights[5];
  private Color mColors[SoundTypesCount];
  private bool mIsInitialized;
  private ThinkerIterator mIterator;
  private Map<int, Object> mSectorMemories;

  private Cvar mXPositionCvar;
  private Cvar mYPositionCvar;
  private CVar mXRadiusCvar;
  private CVar mYRadiusCvar;
  private Cvar mDotSizeCvar;
  private Cvar mOnAutomapCvar;
  private Cvar mMaxDistanceCvar;
  private Cvar mBaseColorCvar;
  private Cvar mColorCvars[SoundTypesCount];
}

class sts_SectorMemory
{
  int time;
  double floorHeight;
  double ceilingHeight;
}
#+end_src

* Test

#+begin_src c :tangle build/SoundToScreenTest/mapinfo.txt

GameInfo { EventHandlers = "stst_EventHandler" }
#+end_src

#+begin_src c :tangle build/SoundToScreenTest/zscript.zs

version 4.13.3

class stst_EventHandler : StaticEventHandler
{
  override void networkProcess(ConsoleEvent command)
  {
    if (command.name == "stst_spawn_imps")
    {
      int count = command.args[0];
      Console.printf("Spawning %d imps...", count);
      for (int i = 0; i < count; ++i)
        Actor.spawn("doomimp", players[consolePlayer].mo.pos + (50, 0, 0));
    }
  }
}
#+end_src

#+begin_src text :tangle build/SoundToScreenTest/commands.txt

wait 2; map map01; wait 2; quit
#+end_src

#+begin_src elisp :exports none
(compile "scons -Q SoundToScreenTest")
#+end_src
