...
See what can be built with command ~scons -h~. It should show three types of build
targets: <Project>, <Project>Test, and <Project>Pack.
- <Project> makes a project directory from the .org file. Such a directory can be
  loaded by the engine.
- <Project>Test tests the project. Some projects have only stubs for tests.
- <Project>Pack makes a .pk3 package for the project. Library projects don't have
  this target.

Example: ~scons FinalCustomDoomPack~

All commands requires that ~PATH~ environment variable has Emacs, tests also require
