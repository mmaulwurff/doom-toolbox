...
OptionMenu ts_Options
{
  ...
  Submenu "View options",   ts_ViewOptions
}
...
class ts_EventHandler : StaticEventHandler
{
  ...
  clearscope ts_ClassList getBlocklist() { return mBlocklist; }
  ...
  ts_ClassList mBlocklist;
  ts_ClassList mFollowers;
  ts_Fader mFader;
}
#+end_src
...
#+begin_src c :noweb-ref options-cvar-getter
int getMinLightLevel() const { return mMinLightLevelCvar.getInt(); }
#+end_src
#+begin_src c :noweb-ref options-cvar-definition
...
class tsView_Text : Actor
{
  static ui void draw(Array<ts_InformationPiece> informationPieces, double alpha)
  {
    ...
    int lineHeight = int(round(aFont.getHeight() * scale));

    foreach (piece : informationPieces)
    {
      string text = toText(piece, options);
      if (text.length() == 0) continue;
      drawText(aFont, text, xCenter, y, alpha, scale);
      y += lineHeight;
    }
  }
#+end_src
...
#+end_src
#+begin_src c :noweb-ref view-options
Slider "Scale", ts_scale, 0.1, 4.0, 0.1, 1
#+end_src

#+begin_src c :tangle build/TargetSpy/zscript/ts_views.zs

  static ui string makeBar(int value, int max, ts_Options options)
  {
...
#+begin_src c :tangle build/TargetSpy/zscript/ts_views.zs
  ...
  static private ui string toText(ts_InformationPiece piece, ts_Options options)
  {
    ...
    case ts_Kind.Bar:         return makeBar(piece.value, piece.max, options);
    }
    return "";
  }
  ...
  static private ui void drawText(Font aFont,
                                  ...
                                  double scale)
  {
    int x = xCenter - int(round(aFont.stringWidth(text) * scale / 2));
...
class OptionMenuItemts_BarPreview : OptionMenuItemStaticText
{
	OptionMenuItemStaticText init(int percent)
  {
    ...
    mLabel = tsView_Text.makeBar(value, 64, options);
    drawLabel(indent, y, Font.CR_White);
    return -1;
	}
