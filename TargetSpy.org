...
1. *Finders* find targets. They must have a name that starts with ~tsFinder_~ and have
   ~static Actor find()~ function.
...
3. *Getters* collect information about a target. They must have a name that starts with
   ~tsGetter_~ and have ~static int, string, int, int makeInformation(Actor)~
...
class ts_EventHandler : StaticEventHandler
{
  ...
  clearscope ts_Classes getFinders() { return mFinderClasses; }
  clearscope ts_Classes getFilters() { return mFilterClasses; }
  clearscope ts_Classes getGetters() { return mGetterClasses; }
  ...
  private void collectInformation()
  {
    ...
    for (int i = 0; i < gettersCount; ++i)
    {
      bool isEnabled = disabledClasses.find(mGetterClasses.classes[i])
        == disabledClasses.size();
      if (isEnabled)
      {
        let [kind, text, value, max] = mGetters[i].call(mCurrentTarget);
        mInformationPieces[i].id    = mGetterClasses.classes[i];
  ...
  private static void warnTooLongList(ts_Classes classes, string listName)
  {
    string disabledClasses = ts_su.join(classes.classes, ",");
  ...
  private bool tryAddFinder(class<Object> aClass)
  {
    ...
    if (finder != NULL)
    {
      ...
      mFinderClasses.classes.push(shorten(aClass.getClassName()));
      mTargets.push(NULL);
    }
    return finder != NULL;
  }
  ...
  private bool tryAddFilter(class<Object> aClass)
  {
    ...
    if (filter != NULL)
    {
      ...
      mFilterClasses.classes.push(shorten(aClass.getClassName()));
    }
    return filter != NULL;
  }
  ...
  private bool tryAddGetter(class<Object> aClass)
  {
    ...
    if (getter != NULL)
    {
      ...
      mGetterClasses.classes.push(shorten(aClass.getClassName()));
...
{
  void fill(ts_Classes classes, string classPrefix, string disabledClassesCvarName)
  {
    mDesc.mItems.clear();
    ...
    for (int i = 0; i < classesCount; ++i)
    {
      ...
      toggle.initialize(classes.classes[i], classPrefix, disabledClassesCvarName);
...
class %s : ts_ClassToggles
{
  override void init(Menu parent, OptionMenuDescriptor descriptor)
  {
    ...
    Super.fill(ts_EventHandler.getInstance().%s(), \"%s\", \"%s\");
  }
