...
class ts_EventHandler : StaticEventHandler
{
  ...
  override void worldTick()
  {
    ...
    findTargets();
    filterTargets();
    collectInformation(chooseCurrentTarget());
    mFader.tick();
  }
  ...
  private Actor chooseCurrentTarget()
  {
  ...
  private void collectInformation(Actor currentTarget)
  {
    ...
    for (int i = 0; i < gettersCount; ++i)
    {
      ...
      if (isEnabled)
      {
        ...
        [mInformationPieces[i].kind,
         ...
         mInformationPieces[i].max] = mGetters[i].call(currentTarget);
      }
    }
  ...
  private void collectGetters()
  {
    ...
    foreach (aClass : AllActorClasses)
    {
      ...
      string shortName = shorten(className);
  ...
  private bool tryAddView(class<Object> aClass)
  {
    ...
    let view = (Function<ui void(Array<ts_InformationPiece>,
                                 ...
                                 RenderEvent,
                                 vector3,
                                 vector2)>)(findFunction(aClass, "draw"));
  ...
  private Function <ui void(Array<ts_InformationPiece>,
                            double,
                            RenderEvent,
                            vector3,
                            vector2)> mView;
                     ...
                     vector2 targetSize)
{
  ...
  foreach (line : lines)
  {
    drawText(aFont, line, position, alpha, scale);
                              ...
                              vector2 targetSize)
{
  ...
  ts_Projector projector = ts_EventHandler.getInstance().getProjector();

  if (place == ts_Options.PlaceAboveTarget) targetPosition.z += targetSize.y;
  return projector.makePosition(event, targetPosition);
}
#+end_src
...
static private ui void drawText
  (Font aFont, string text, vector2 position, double alpha, double scale)
{
  position.x -= int(round(aFont.stringWidth(text) * scale / 2));
