...
class ts_EventHandler : StaticEventHandler
{
  ...
  private bool tryAddView(class<Object> aClass)
  {
    ...
    {
      mViews.push(view);
      mViewClasses.push(shorten(aClass.getClassName()));
    }
    return view != NULL;
  }
...
TODO: add separate configurations for different bars by identifier.
...
#+end_src
#+begin_src c :noweb-ref ts_Options.functions
  ts_BarOptions getBarOptions() { return mBarOptions; }
#+end_src
#+begin_src c :noweb-ref ts_Options.members
  private ts_BarOptions mBarOptions;
#+end_src
#+begin_src c :noweb-ref options-cvar-initialization
  (mBarOptions = new("ts_BarOptions")).initialize();
#+end_src
#+begin_src c :tangle build/TargetSpy/cvarinfo.txt
...
user string ts_text_characters = "❤♡█▓▒░☻☹⛧⛤☣✚⛊▬●⚰☐☒☑⚙";
#+end_src
#+begin_src c :tangle build/TargetSpy/menudef.txt
OptionMenu ts_BarOptions
{
  ...
  TextField "Characters",    ts_text_characters
...
static private ui string toText(ts_InformationPiece piece, ts_Options options)
{
  ...
  case ts_Kind.Bar:
    return makeBar(piece.value, piece.max, options.getBarOptions());
  }
  return "";
}
...
class OptionMenuItemts_BarPreview : OptionMenuItemStaticText
{
	OptionMenuItemStaticText init(int percent)
  {
    ...
    ts_BarOptions options =
      ts_EventHandler.getInstance().getOptions().getBarOptions();
