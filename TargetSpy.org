...
class tsView_Frame : Actor
{
                      ...
                      vector2 targetSize)
  {
    ...
    int color = getColor(options, informationPieces);
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      vector2 position = (xs[i], ys[i]);
      ts_ViewUtils.drawText(aFont, part, width, position, alpha, scale, color);
    }
  }
  ...
  static ui int getColor(ts_Options options, Array<ts_InformationPiece> pieces)
  {
    if (!options.isFrameUsingHealthBarColors()) return Font.CR_Gray;
...
mFrameWithHealthColors  = Cvar.findCvar("ts_frame_using_health_bar_colors");
#+end_src
#+begin_src c :tangle build/TargetSpy/cvarinfo.txt
