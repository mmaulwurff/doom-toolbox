...
class ts_EventHandler : StaticEventHandler
{
  ...
  {
    if (event.player == consolePlayer && event.name == "ts_swap_getters")
      swapGetters(event.args[0], event.args[1]);
  }
  ...
  private void swapGetters(int i, int j)
  {
    int gettersCount = mGetters.size();
    i = clamp(i, 0, gettersCount - 1);
    j = clamp(j, 0, gettersCount - 1);
    if (i == j) return;

    {
      let temp = mGetters[i];
      mGetters[i] = mGetters[j];
      mGetters[j] = temp;
    }
    {
      let temp = mGetterClasses[i];
      mGetterClasses[i] = mGetterClasses[j];
      mGetterClasses[j] = temp;
    }
  }
  ...
  private Function<ui void(Array<ts_InformationPiece>, double)> mView;
