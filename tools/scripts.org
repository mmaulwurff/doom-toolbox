...
Utility scripts for DoomToolbox development.

* Include

Lists includes from ~org-file~ by ~regexp~. ~regexp~ must contain a group with
zscript file name. Example: ~"build/MyMod/\\(zscript/.*zs\\)"~.
...
Peculiar behavior for command files executed by UZDoom with ~exec~: ~wait~ command
from a line doesn't affect the time of execution of the next line. We need a
convenient way to force command sequence. Use ~<<wait(N)>>~ instead of ~wait N~.
