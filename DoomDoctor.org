...
class dd_Test : Clematis
{
  ...
  {
    Describe("Doom Doctor tests");
    let bufferedConsole = dd_BufferedConsole.getInstance();
#+end_src
# SPDX-SnippetEnd
...
#+begin_src c :tangle build/DoomDoctorTest/zscript.txt
{
  it("logged: OnRegister", Assert(bufferedConsole.contains("OnRegister")));
}
#+end_src

**** OnUnregister
- TODO: make TestRunner run some tests at specified events
