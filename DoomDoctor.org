...
TODO: rename TestRunner? Or move module loading script elsewhere.
#+name: modules
...
*** OnUnregister
- TODO: make TestRunner restart the map for some tests
#+begin_src c :tangle build/DoomDoctor/zscript.txt
override void OnUnregister()
{
...
The player is killed by console commands in [[Launch][Launch]] section.
...
The player is resurrected by console commands in [[Launch][Launch]] section.
...
The player is damaged by console commands in [[Launch][Launch]] section.
...
#+begin_src elisp
(let ((definitions ""))
  (dotimes (i (1+ only-once-id))
    (setq definitions (format "%sprivate bool mOnlyOnceFlag%d;\n" definitions i)))
...
- Move file structure QA to a separate file, add link here.
