...
⚠ Warning! Don't run scripts from the sources you don't trust.

The breakdown of the script above:
- launch Emacs with the SomeFile.org file:
  ...
  - ~--eval~ - run the following Elisp commands:
    ...
    - ~require 'ob-tangle~ - load the Emacs module for separating .org files;
    ...
    - ~org-babel-tangle~ - move the parts of the .org file into separate files.
