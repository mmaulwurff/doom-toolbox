...
- Accuracy. Don't expect physical simulation level of precision. Some collisions
  cannot be cleanly handled.
...
class hi_EventHandler : StaticEventHandler
{
  ...
  override void worldTick()
  {
    ...
    for (int i = 0; i < missileCount; ++i)
    {
      ...
      if (!missile || !missile.target || missile.vel != mNullVector) continue;
    ...
    getMissileHitData(Actor source, Actor missile, out ts_HitData result)
  {
    ...
    else if (missile.blockingLine)
    {
      ...
      FLineTraceData traceData;
      vector2 lineCenter = (missile.blockingLine.v1.p + missile.blockingLine.v2.p) / 2;
      double  offsetZ    = missile.height / 2;
      vector2 dir        = lineCenter - missile.pos.xy;
      double  angle      = atan2(dir.y, dir.x);
      double  distance   = missile.blockingLine.delta.length() + missile.radius * 2;
      bool    isHit =
    ...
    result.isHit = true;
    result.normal = normal;
    // This isn't accurate if missile trajectory is not straight.
    result.incidence = level.vec3Diff(source.pos, missile.pos).unit();
