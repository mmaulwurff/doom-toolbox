...
#include "zscript/cd_PlainTranslator.zs"
#include "zscript/cd_StringUtils.zs"
#+end_src

Note: ~cd_PlainTranslator.zs~ must be installed externally.
...
lump, where settings are added to ~cd_Menu~, for example in a submenu. Settings
...
- Setting labels in some item types are made directly translatable. See the note in
  language.txt in [[Menus]] section.
