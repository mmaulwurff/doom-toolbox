...
FinalCustomDoom provides gameplay customization. It can be used to increase or
decrease the difficulty.

FinalCustomDoom is a successor to Ultimate Custom Doom and Custom Doom.

FinalCustomDoom is a part of [[https://github.com/mmaulwurff/doom-toolbox/][DoomToolbox]].
...
// <<license>>
#+end_src
#+begin_src c :tangle ../build/FinalCustomDoom/language.txt :exports none
// <<license>>
#+end_src
...
{
  <<OptionMenuTitle("$cd_Player")>>
  <<OptionMenuNote("$cd_ZeroDisables")>>

  TextField   "$cd_WeaponDamageMultiplier", "cd_Player:weaponDamage:Immediately"
  TextField   "$cd_TakenDamageMultiplier" , "cd_Player:takenDamage:Immediately"
  StaticText  ""
  NumberField "$cd_StartHealth"           , "cd_Player:startHealth:OnPlayerStarted"
  NumberField "$cd_StartArmor"            , "cd_Player:startArmor:OnPlayerStarted"
  TextField   "$cd_SavePercent"           , "cd_Player:savePercent"
  StaticText  ""
  NumberField "$cd_HealthMax"             , "cd_Player:maxHealth:Immediately"
  TextField   "$cd_SpeedMultiplier"       , "cd_Player:speedMultiplier:Immediately"
  TextField   "$cd_JumpHeightMultiplier"  , "cd_Player:jumpMultiplier:Immediately"
  StaticText  ""
  TextField   "$cd_FrictionMultiplier"    , "cd_Player:friction:Immediately"
  TextField   "$cd_SelfDamageMultiplier"  , "cd_Player:selfDamage:Immediately"
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_Actors")>>
  <<OptionMenuNote("$cd_ZeroDisables")>>

  StaticText  "$cd_Enemies"         , White
  TextField   "$cd_HealthMultiplier", "cd_Actors:enemyHealth:OnActorSpawned"
  NumberField "$cd_HealthMax"       , "cd_Actors:enemyHealthMax:OnActorSpawned"
  TextField   "$cd_SpeedMultiplier" , "cd_Actors:enemySpeed:OnActorSpawned"
  StaticText  ""
  StaticText  "$cd_Friends"         , White
  TextField   "$cd_HealthMultiplier", "cd_Actors:friendHealth:OnActorSpawned"
  NumberField "$cd_HealthMax"       , "cd_Actors:friendHealthMax:OnActorSpawned"
  TextField   "$cd_SpeedMultiplier" , "cd_Actors:friendSpeed:OnActorSpawned"
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_PermanentPowerups")>>

  Option "$cd_Buddha"           , "cd_Powerups:buddha:Periodically"           , OnOff
  Option "$cd_Damage"           , "cd_Powerups:damage:Periodically"           , OnOff
  Option "$cd_DoubleFiringSpeed", "cd_Powerups:doubleFiringSpeed:Periodically", OnOff
  Option "$cd_Drain"            , "cd_Powerups:drain:Periodically"            , OnOff
  Option "$cd_Flight"           , "cd_Powerups:flight:Periodically"           , OnOff
  Option "$cd_Frightener"       , "cd_Powerups:frightener:Periodically"       , OnOff
  Option "$cd_Ghost"            , "cd_Powerups:ghost:Periodically"            , OnOff
  Option "$cd_HighJump"         , "cd_Powerups:highJump:Periodically"         , OnOff
  Option "$cd_InfiniteAmmo"     , "cd_Powerups:infiniteAmmo:Periodically"     , OnOff
  Option "$cd_Invisibility"     , "cd_Powerups:invisibility:Periodically"     , OnOff
  Option "$cd_Invulnerability"  , "cd_Powerups:invulnerability:Periodically"  , OnOff
  Option "$cd_IronFeet"         , "cd_Powerups:ironFeet:Periodically"         , OnOff
  Option "$cd_LightAmp"         , "cd_Powerups:lightAmp:Periodically"         , OnOff
  Option "$cd_Mask"             , "cd_Powerups:mask:Periodically"             , OnOff
  Option "$cd_Minotaur"         , "cd_Powerups:minotaur:Periodically"         , OnOff
  Option "$cd_Morph"            , "cd_Powerups:morph:Periodically"            , OnOff
  Option "$cd_Protection"       , "cd_Powerups:protection:Periodically"       , OnOff
  Option "$cd_Regeneration"     , "cd_Powerups:regeneration:Periodically"     , OnOff
  Option "$cd_Scanner"          , "cd_Powerups:scanner:Periodically"          , OnOff
  Option "$cd_Shadow"           , "cd_Powerups:shadow:Periodically"           , OnOff
  Option "$cd_Speed"            , "cd_Powerups:speed:Periodically"            , OnOff
  Option "$cd_Strength"         , "cd_Powerups:strength:Periodically"         , OnOff
  Option "$cd_Targeter"         , "cd_Powerups:targeter:Periodically"         , OnOff
  Option "$cd_TimeFreeze"       , "cd_Powerups:timeFreeze:Periodically"       , OnOff
  Option "$cd_Torch"            , "cd_Powerups:torch:Periodically"            , OnOff
  Option "$cd_WeaponLevel2"     , "cd_Powerups:weaponLevel2:Periodically"     , OnOff
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_HealthRegeneration")>>
  <<OptionMenuNote("$cd_ZeroDisables")>>

  NumberField "$cd_Amount", "cd_HealthRegeneration:amount:Periodically"
  Option      "$cd_Type"  , "cd_HealthRegeneration:type", cd_RegenerationType
  NumberField "$cd_Periodseconds", "cd_HealthRegeneration:period"
  StaticText  ""
  NumberField "$cd_Min", "cd_HealthRegeneration:min"
  NumberField "$cd_Max", "cd_HealthRegeneration:max"
  StaticText  ""
  Textfield   "$cd_SoundEffectVolume"    , "cd_HealthRegeneration:sound"
  TextField   "$cd_VisualEffectIntensity", "cd_HealthRegeneration:visual"
  ColorPicker "$cd_VisualEffectColor"    , "cd_HealthRegeneration:color"
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_ArmorRegeneration")>>
  <<OptionMenuNote("$cd_ZeroDisables")>>

  NumberField "$cd_Amount", "cd_ArmorRegeneration:amount:Periodically"
  Option      "$cd_Type"  , "cd_ArmorRegeneration:type", cd_RegenerationType
  NumberField "$cd_PeriodSeconds", "cd_ArmorRegeneration:period"
  StaticText  ""
  NumberField "$cd_Min", "cd_ArmorRegeneration:min"
  NumberField "$cd_Max", "cd_ArmorRegeneration:max"
  StaticText  ""
  TextField   "$cd_SoundEffectVolume"    , "cd_ArmorRegeneration:sound"
  TextField   "$cd_VisualEffectIntensity", "cd_ArmorRegeneration:visual"
  ColorPicker "$cd_VisualEffectColor"    , "cd_ArmorRegeneration:color"
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_AmmoRegeneration")>>
  <<OptionMenuNote("$cd_ZeroDisables")>>

  NumberField "$cd_Amount"          , "cd_AmmoRegeneration:amount:Periodically"
  NumberField "$cd_PeriodSeconds"   , "cd_AmmoRegeneration:period"
  Option      "$cd_BackpackRequired", "cd_AmmoRegeneration:backpackRequired", OnOff
  StaticText  ""
  TextField   "$cd_SoundEffectVolume"    , "cd_AmmoRegeneration:sound"
  TextField   "$cd_VisualEffectIntensity", "cd_AmmoRegeneration:visual"
  ColorPicker "$cd_VisualEffectColor"    , "cd_AmmoRegeneration:color"
}
#+end_src
...
{
  <<OptionMenuTitle("$cd_Commands")>>
  <<OptionMenuNote("$cd_NotAppliedInGame")>>
...
OptionMenu cd_Menu protected
{
  ...
  Submenu    "$cd_Player"  , cd_Player
  Submenu    "$cd_Actors"  , cd_Actors
  Submenu    "$cd_PermanentPowerups", cd_Powerups
  StaticText ""
  StaticText "$cd_RegenerationDegeneration", White
  Submenu    "$cd_Health"  , cd_HealthRegeneration
  Submenu    "$cd_Armor"   , cd_ArmorRegeneration
  Submenu    "$cd_Ammo"    , cd_AmmoRegeneration
  StaticText ""
  Submenu    "$cd_Commands", cd_Commands
}
...
#+begin_src c :tangle ../build/FinalCustomDoom/language.txt
...
<<../tools/scripts.org:include("add-ons/FinalCustomDoom.org", "build/FinalCustomDoom/\\(zscript/.*zs\\)")>>

#include "zscript/cd_StringUtils.zs"
#+end_src

#+begin_src text :tangle ../build/tmp/FinalCustomDoom.txt :exports none
<<../tools/scripts.org:copy-media("FinalCustomDoom")>>
#+end_src
...
class cd_EventHandler : StaticEventHandler
{
  ...
  {
    if (cd_su.beginsWith(event.name, "cd"))
    {
...
class cd_EffectsBase play
{
  ...
  // 0 to 1 multipliers: 0.0 acts as 1.0, both meaning it does nothing.
...
lump, where settings are added to ~cd_Menu~, for example a submenu. Settings
implementation provides in-game effects and is written in ZScript.

Settings defined in a FCD extension don't have an entry in cvarinfo lump. They're
stored, reset to defaults, and backed up to profiles together with FCD settings.

Important note: FCD extensions don't depend on FCD code-wise. This means that they
can be loaded without errors even without FCD.

Options in ~cd_Menu~ and its submenus don't behave like normal options. The
differences are:
- Some item types are transformed into Custom Doom settings:
...
- Instead of a Cvar, a command is specified in format ~"Class:Function:EffectTime"~:
  - ~Class~ is ZScript class name that contains Function. Attention: class name must
    start with ~cd~.
  - ~Function~: ZScript function name in Class. It must take a string as a parameter,
    and have return type void (meaning it returns nothing).
  - ~EffectTime~: one of : ~Immediately~, ~OnPlayerStarted~, ~OnActorSpawned~,
    ~Periodically~, or left out.
...
#+begin_src text :tangle ../build/FinalCustomDoomTest/commands.txt
wait 2; map map01
wait 4; summon doomimp
wait 6; openmenu cd_menu
wait 8; quit
#+end_src
