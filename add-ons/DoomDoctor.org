...
#+begin_src text :exports none :noweb-ref CC
SPDX-FileCopyrightText: © 2021 Alexander Kromm (m8f) <mmaulwurff@gmail.com>
...
// <<CC>>
#+end_src
#+begin_src c :tangle ../build/DoomDoctor/keyconf.txt :exports none
// <<CC>>
#+end_src
...
// <<GPL>>
#+end_src
#+begin_src c :tangle ../build/DoomDoctor/zscript/dd_Logger.zs :exports none
// <<GPL>>
#+end_src
#+begin_src c :tangle ../build/DoomDoctor/zscript/dd_troublemaker.zs :exports none
// <<GPL>>
#+end_src
...
- Don't trigger more than one event at once to speed up tests. The events must not
  interfere with each other.
- Warp and use commands are supposed to activate the [[https://github.com/fragglet/miniwad][Miniwad]] end level button.

#+begin_src text :tangle ../build/DoomDoctorTest/commands.txt
...
wait 16; resurrect; warp 60 -128 0; +use;
...
<<../tools/scripts.org:include("add-ons/DoomDoctor.org", "build/DoomDoctor/\\(zscript/.*zs\\)")>>

#include "zscript/dd_Fonts.zs"
#include "zscript/dd_StringUtils.zs"
#include "zscript/dd_VmAbortReporter.zs"
#+end_src
...
Alias dd_show_fonts            "netevent dd_show_fonts"
#+end_src
...
#+begin_src c :tangle ../build/DoomDoctor/zscript/dd_troublemaker.zs
...
class dd_Troublemaker : EventHandler
{
  ...
  override void NetworkProcess(ConsoleEvent event)
  {
    ...
    if (!dd_su.beginsWith(command, "dd_")) return;
    ...
    else if (command == "dd_show_fonts") showFonts();
  }
  ...
  override void renderOverlay(RenderEvent event)
  {
    ...
    string lowerText = "sphinx of black quartz, judge my vow!";
    string upperText = lowerText.makeUpper();

    Font fallbackFont = dd_Fonts.findSmall();

    int y = 0;
    int fontsCount = fonts.size();
    for (int i = 0; i < fontsCount; ++i)
    {
      ...
      if (font == NULL)
      {
        Screen.drawText(fallbackFont, Font.CR_White, 0, y, fontName .. ": NULL");
        y += fallbackFont.getHeight();
        continue;
      }

      Screen.drawText(font, Font.CR_White, 0, y, fontName .. ": " .. lowerText);
      y += font.getHeight();
      Screen.drawText(font, Font.CR_White, 0, y, fontName .. ": " .. upperText);
      y += font.getHeight();
    }
  }
  ...
  {
    mIsFontsShown = !mIsFontsShown;
  }

  private bool mIsScheduledSpawnedThingIsNull;
  private bool mIsFontsShown;
}
#+end_src
...
#+name: dd_logging_disable
#+begin_src elisp :exports none :noweb-prefix no
(org-trim (string-replace "\n" " " "

ResetCvar dd_logging_engine_events_enabled;
ResetCvar dd_logging_replace_events_enabled;
ResetCvar dd_logging_world_events_enabled;
ResetCvar dd_logging_player_events_enabled;
ResetCvar dd_logging_process_events_enabled;
"))
#+end_src

#+begin_src c :tangle ../build/DoomDoctor/keyconf.txt

Alias dd_logging_disable "<<dd_logging_disable()>>"
#+end_src
...
#+begin_src c :tangle ../build/DoomDoctor/zscript/dd_Logger.zs
...
Notes
- The following events aren't logged, because nothing interesting can change here:
  ~RenderOverlay~, ~RenderUnderlay~, ~UiTick~, ~PostUiTick~, ~InputProcess~,
  ~UiProcess~.
- Events can't be destroyed, so event parameters are never NULL.
...
TODO: move checks to somewhere where they're move visible.
  ...
  foreach (aClass : AllClasses)
    {
      ...
      // Ignore the test class.
      string loggerTestClass = "dd_LoggerTestRunner";
      if (instance is loggerTestClass) continue;
