...
It's designed to be compatible with any monster and weapon mod, and any map.
...
OptionMenu aas_ScreenshotOptions
{
  ...
  Option     "$AAS_EVENT_LEVEL_START"  , aas_screenshot_on_level_start     , OnOff
...
OptionMenu aas_VoiceOptions
{
  ...
  Option     "$AAS_EVENT_LEVEL_START"  , aas_voice_on_level_start     , OnOff
...
|  11 | ~important~       | true    | Something important!   |
|  12 | ~level_start~     | false   | Level started          |
|  13 | ~one_percent~     | false   | Last health percent!   |
|  14 | ~enemies_10~      | false   | Enemies 10% down       |
|  15 | ~enemies_20~      | false   | Enemies 20% down       |
|  16 | ~enemies_30~      | false   | Enemies 30% down       |
|  17 | ~enemies_40~      | false   | Enemies 40% down       |
|  18 | ~enemies_50~      | false   | Enemies 50% down       |
|  19 | ~enemies_60~      | false   | Enemies 60% down       |
|  20 | ~enemies_70~      | false   | Enemies 70% down       |
|  21 | ~enemies_80~      | false   | Enemies 80% down       |
|  22 | ~enemies_90~      | false   | Enemies 90% down       |
|-----+-------------------+---------+------------------------|
...
<<list-events("AAS_EVENT_%2$s \= \"%4$s\";")>>
#+end_src
...
AAS_EVENT_manual       = "Save manual";
AAS_EVENT_important    = "Algo importante!";
AAS_EVENT_level_start  = "Nível começado";
...
class aas_event_source play
{
  static aas_event_source of()
  {
    ...
    result._saver = aas_saver.of(result._scheduler, result._clock, last_save_time);

    Array<aas_event_handler> handlers =
    {
      result._saver,
      aas_logger.of(aas_log_function.of(), "aas_console_log_level"),
      aas_logger.of(aas_print_function.of(aas_Fonts.findSmall()), "aas_screen_level"),
      result._voice,
      aas_screenshot_maker.of(result._scheduler)
    };
  ...
  void on_thing_spawned(Actor item)
  {
    ...
    if (aas_su.beginsWith(class_name, "aas_"))
    {
      if (aas_su.beginsWith(class_name, "aas_pickup_"))
        aas_token(Actor.spawn("aas_token", item.pos)).init(aas_event.important,
                                                           _handler,
                                                           NULL);

      else if (aas_su.beginsWith(class_name, "aas_instant_"))
        on_event(aas_event.important);

      else if (aas_su.beginsWith(class_name, "aas_silent_instant_"))
        _saver.on_event(aas_event.important);
  ...
  private aas_event_handler _saver;
...
class aas_function abstract
{
  abstract void f1(string s);
}
#+end_src
...
class aas_token : Inventory
{
  ...
  aas_token init(int type, aas_event_handler handler, Actor origin)
  {
    _event_type = type;
    _handler    = handler;
    _origin     = origin;
    _connected  = origin != NULL;
  ...
  override void tick()
  {
    ...
    setOrigin(_origin.pos, true);
    a_setSize(_origin.radius, _origin.height);
  }
...
// a message like "Something important!").
// In settings, saves caused by tokens fall into "Important" category.
//
...
{
  // Universal Autoautosave Tokens don't have to exist after being spawned.
  States
  {
  Spawn:
    TNT1 A 0;
...
// This is an example of Instant Universal Autoautosave Token.
//
// How to use:
...
{
  // These tokens don't have to exist after being spawned.
  States
  {
  Spawn:
    TNT1 A 0;
...
// This is an example of Silent Instant Universal Autoautosave Token.
//
// How to use:
// 1. Define a class that starts with "aas_instant_";
// 2. Spawn this class, and autoautosave happens immediately, without playing
//    the voice notification, and without printing to console and to screen.
class aas_silent_instant_example : Actor
{
  // These tokens don't have to exist after being spawned.
  States
  {
...
class aas_event_handler abstract
{
  ...
  abstract void on_event(int event_type);
}
#+end_src
...
#include "zscript/aas_StringUtils.zs"
#+end_src
