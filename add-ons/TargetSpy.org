...
TargetSpy supports four types of extension classes:
...
3. *Getters* collect information about a target. They must have a name that starts
   ...
   Additionally, a getter can have ~static Font getFont(Actor)~ function to select a
   font for an actor.
...
4. *Views* show information on screen. They must have a name that starts with
   ...
   ~vector2 targetSize)~ function. See [[Information piece][here]] for reference what ~ts_InformationPiece~
   contains.

Note that Finders, Filters, and Getters classes don't depend on TargetSpy types in
any way. Theyy can be included in other mods to provide TargetSpy
integration when loaded together.

Extension classes aren't meant to be instantiated, only their static functions are
used. Extension classes can have a translatable user string defined in ~LANGUAGE~
lump. The string key is the class name. Extension classes are automatically
...
implemented as extensions. They can be used as examples for implementing custom
extensions.
...
v3.1.2:
- fix: replace broken extension replacement with working
- fix: made font finding more robust
- fix: don't draw frames from previous map
- fix: fading text shows dead health instead of before death

v3.1.0:
- new:
  - keys to turn on/off and hold
  - option for health color for all lines
  - font customization
  - extension configuration
  - extension replacing
- fixed:
  - frame position
  - bar values are rounded up now
  - armor bar
  - ~vid_rendermode~ is optional again
  - user name for players
  - disabled on automap
...
OptionMenu ts_Options
{
  ...
  Option "TargetSpy", ts_on, OnOff

  StaticText ""
  StaticText "$ts_Toggles"
  Submenu "$ts_Finders", ts_FinderToggles
  Submenu "$ts_Filters", ts_FilterToggles
  Submenu "$ts_Getters", ts_GetterToggles
  Submenu "$ts_Views",   ts_ViewToggles

  StaticText ""
  Submenu "$ts_FilterOptions",    ts_FilterOptions
  Submenu "$ts_GettersOrder",     ts_GettersOrder
  Submenu "$ts_TextViewOptions",  ts_TextViewOptions
  Submenu "$ts_FrameOptions",     ts_FrameOptions
  Submenu "$ts_CrosshairOptions", ts_CrosshairOptions
}
#+end_src
...
*** Silent health and max health

This getter is special. It's intended to:
...
#+begin_src js :tangle ../build/TargetSpy/ts_config.json
{
...
class tsGetter_Tag : Actor
{
  ...
  //{
  //  return anActor is "Zombieman" ? BigFont : NewSmallFont;
  //}
}
#+end_src
...
{
  Title "$ts_TextViewOptions"

  Slider "$ts_Scale",      ts_scale, 0.1, 4.0, 0.1, 1
  Slider "$ts_Xposition",  ts_x_position, 0, 1.0, 0.01, 2
  Slider "$ts_Yposition",  ts_y_position, 0, 1.0, 0.01, 2
  Option "$ts_Place",      ts_place, ts_Places
  Option "$ts_Background", ts_background, OnOff
  Option "$ts_UseHealthBarColors", ts_using_health_bar_colors, OnOff

  StaticText ""
  Submenu "$ts_DefaultBarOptions", ts_DefaultBarOptions
  Submenu "$ts_HealthBarOptions",  ts_HealthBarOptions
  Submenu "$ts_ArmorBarOptions",   ts_ArmorBarOptions
}
...
{
  Title "$ts_FrameOptions"

  Slider "$ts_Scale",  ts_frame_scale, 0.1, 4.0, 0.1, 1
  Option "$ts_UseHealthBarColors", ts_frame_using_health_bar_colors, OnOff

  StaticText ""
  Option "$ts_CustomCharacters", ts_frame_custom_enabled, OnOff

  StaticText ""
  Slider "$ts_Style", ts_frame_selected, 0, 7, 1, 0, ts_frame_custom_enabled, 1

  StaticText ""
  StaticText "$ts_CustomCharacters"
  TextField "$ts_TopLeft",  ts_frame_custom_top_left,  ts_frame_custom_enabled, 0
  TextField "$ts_TopRight", ts_frame_custom_top_right, ts_frame_custom_enabled, 0
  TextField "$ts_BottomLeft", ts_frame_custom_bottom_left, ts_frame_custom_enabled, 0
  TextField "$ts_BottomRight", ts_frame_custom_bottom_right, ts_frame_custom_enabled, 0
}
#+end_src
...
class tsView_Crosshair : Actor
{
                      ...
                      vector2 targetSize)
  {
    ...
    Font    aFont  = eventHandler.getDefaultFont();
...
{
  Title "$ts_CrosshairOptions"

  Option "$ts_AlwaysEnabled", ts_crosshair_always, OnOff
  Option "$ts_UseHealthBarColors", ts_crosshair_health, OnOff

  StaticText ""
  Slider "$ts_Height", ts_crosshair_height, 0.5, 40, 0.1
  Slider "$ts_Scale",  ts_crosshair_scale,  0.5,  4, 0.1

  StaticText ""
  TextField "$ts_Top",    ts_crosshair_top
  TextField "$ts_Center", ts_crosshair_center
  TextField "$ts_Bottom", ts_crosshair_bottom
}
#+end_src
...
class ts_EventHandler : StaticEventHandler
{
  override void onRegister()
  {
    ...
    mDefaultFont   = ts_Fonts.findSmall();
  ...
  {
    if (players[consolePlayer].mo == NULL) return;
    if (isAllInformationEmpty()) return;
  ...
  clearscope Font getDefaultFont() { return mDefaultFont; }
  ...
  {
    Actor target = mCurrentTarget.target;

    if (target == NULL)
    {
      if (mFader.getAlpha(1.0) <= 0.0)
      {
        clearInformation();
        mOldTarget = NULL;
        return;
      }
      else if (mOldTarget != NULL)
      {
        target = mOldTarget;
      }
      else
    ...
    for (int i = 0; i < gettersCount; ++i)
    {
      ...
      if (isEnabled)
      {
        ...
        [mInformationPieces[i].kind,
         ...
         mInformationPieces[i].max] = mGetters[i].call(target);
        mInformationPieces[i].font = mFontGetters[i]
          ? mFontGetters[i].call(target)
          : mDefaultFont;
      }
    }
  }
  ...
  private bool isAllInformationEmpty() const
  {
    foreach (piece : mInformationPieces)
      if (piece.kind != ts_Kind.Empty) return false;
  ...
  {
    // This class replaces something: skip, it's handled on replacee side.
    if (getReplacee(aClass) != aClass) return true;

    // If this class is replaced, use that replacement but keep the original name.
    string className = aClass.getClassName();
    aClass = getReplacement(aClass);
...
class ts_Projector
{
  ...
  void prepare()
  {
    // Projection type detection isn't quite right here.
...
{
  Title '$ts_%1$s options'

  Slider '$ts_FilledCharacter', ts_%1$s_filled_character_index, 0, 99, 1, 0
  ts_HighlightedCharacter       ts_%1$s_filled_character_index, %1$s
  Slider '$ts_EmptyCharacter',  ts_%1$s_empty_character_index, 0, 99, 1, 0
  ts_HighlightedCharacter       ts_%1$s_empty_character_index, %1$s
  TextField '$ts_Characters',   ts_%1$s_characters
  ...
  StaticText ''
  StaticText '$ts_ColorsForFilledCharacters'
  ...
  StaticText ''
  StaticText '$ts_ColorsForEmptyCharacters'
...
Configuration is a way to customize how TargetSpy processes finders, filters,
getters and views. By default these classes are enabled or disabled by user choice.
Configuration can force them to be on or off, and replace them with another class.
If a class is forced on, it's always active and hidden from option menus.
If a class is forced off, it's always disabled and hidden from option menus.

More than one configuration may be loaded at once. If more than one configuration
configures a class, the last loaded configuration wins.

Configuration lump has name ~ts_config~ and is in JSON object format.
Key is full class name, value is "off", "on", "user", or another class name.
"user" can be used to override previous configuration.

Example:

#+begin_src c :noweb-ref zscript-test
...
#+begin_src js :tangle ../build/TargetSpyTest/ts_config.json
{
  "tsFilter_Abc": "on",
  "tsGetter_Def": "off",
  "tsGetter_Health": "user",
  "tsGetter_Tag": "tsGetter_TestTag"
}
#+end_src

If a replaced class must be turned on or off, a second configuration is needed:

#+begin_src js :tangle ../build/TargetSpyTest/ts_config.2.json
{
  "tsGetter_Tag": "on"
}
#+end_src
...
clearscope class<Actor> getReplacement(class<Actor> aClass)
{
  string className = aClass.getClassName();
  string lowerClassName = className.makeLower();
  let [replacement, hasReplacement] = mReplacements.checkValue(lowerClassName);
  class<Actor> result = replacement;
  return hasReplacement && result ? result : aClass;
}

clearscope class<Actor> getReplacee(class<Actor> aClass)
{
  string className = aClass.getClassName();
  string lowerClassName = className.makeLower();
  let [replacee, hasReplacee] = mReplacees.checkValue(lowerClassName);
  class<Actor> result = replacee;
  return hasReplacee && result ? result : aClass;
}
...
private void collectConfigurations()
{
       ...
       i = Wads.findLump(lumpName, i + 1))
  {
    ...
    while (iterator.next())
    {
      ...
      {
        // TODO: make key lower in mConfiguration too.
        string lowerKey = key.makeLower();
        mReplacements.insert(lowerKey, value);
        mReplacees.insert(value, lowerKey);
      }
    }
  }
}
                                   ...
                                   out Array<string> disabledClasses)
{
  ...
  {
    if (!ts_su.beginsWith(className, prefix)) continue;
...
private Map<string, string> mReplacees;
#+end_src
...
<<../tools/scripts.org:include("add-ons/TargetSpy.org", "build/TargetSpy/\\(zscript/.*zs\\)")>>

#include "zscript/ts_Fonts.zs"
