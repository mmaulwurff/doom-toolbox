...
Configuration is a way to customize how TargetSpy processes finders, filters, getters
and views. By default these classes are enabled or disabled by user choice.
Configuration can force them to be on or off. If a class is forced on, it is always
active and hidden from option menus. If a class is forced off, it is always disabled
and hidden from option menus.
...
#+begin_src c :tangle ../build/TargetSpyTest/mapinfo.txt
...
#+begin_src ini :tangle ../build/TargetSpyTest/language.txt
