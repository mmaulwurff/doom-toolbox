                    ...
                    vector2 targetSize)
{
  ...
  for (int i = 0; i < linesCount; ++i)
  {
    ...
    ts_ViewUtils.drawText(fonts[i], lines[i], lineWidths[i], position, alpha, scale);
...
Configuration is in JSON object format. Key is full class name, value is "off", "on",
or "user". "user" can be used to override previous configuration.
...
private clearscope void applyConfiguration(string prefix,
