...
#+begin_quote
Make anything into a slaughtermap, or make nuts.wad not melt your CPU, all from the
reach of a single slider.
#+end_quote
...
- Resurrection of automatically killed enemies (~x5_raise_divided~ Cvar, off by
  default).
...
- Suggested max enemy amount: no, because it would limit enemy numbers
  disproportionately to the map size.
...
- It may not be possible to get 100% kills in maps with single-use monster
  teleporters.

10.5x is a part of [[https://github.com/mmaulwurff/doom-toolbox/][DoomToolbox]].
...
// <<license>>
#+end_src
#+begin_src text :tangle ../build/10.5x/menudef.txt :exports none
// <<license>>
#+end_src
...
class x5_EventHandler : EventHandler
{
  ...
  {
    if (!x5_su.beginsWith(event.name, "x5_")) return;
...
class x5_Killer : Actor
{
  ...
  {
    return level.vec3Offset(watched.pos, (0, 0, watched.height * 1.5));
  }
...
#+begin_src text :tangle ../build/10.5x/menudef.txt

AddOptionMenu OptionsMenu       { X5Slider "", x5_multiplier, 0, 10.5, 0.1, 1 }
AddOptionMenu OptionsMenuSimple { X5Slider "", x5_multiplier, 0, 10.5, 0.1, 1 }
#+end_src
...
class OptionMenuItemX5Slider : OptionMenuItemSlider
{
  ...
  {
    mLabel = (val == 0)
      ? StringTable.localize("$X5_000").." 10.5x \cg⨂"
      : "10.5x \cg⨂";
  }
}
#+end_src
...
<<../tools/scripts.org:include("add-ons/10.5x.org", "build/10.5x/\\(zscript/.*zs\\)")>>

#include "zscript/x5_StringUtils.zs"
#+end_src

* Sprites

sprites/m8rda0.png:
#+attr_HTML: :alt 10.5x enemy death marker
[[file:../media/10.5x/sprites/m8rda0.png]]

#+begin_src text :tangle ../build/tmp/10.5x.txt :exports none

<<../tools/scripts.org:copy-media("10.5x")>>
#+end_src
...
class x5t_Test : StaticEventHandler
{
  ...
  override void networkProcess(ConsoleEvent event)
  {
    ...
    else if (x5_su.beginsWith(event.name, "x5t_expect"))
    {
  ...
  override void worldLoaded(WorldEvent event)
  {
    int width  = int(getDefaultByType('DoomImp').radius * 2);
    int yBegin = -2 * width;
    int yEnd   = 2 * width;
    int x      = 100;
    int y      = yBegin;
...
class x5t_Quoter : EventHandler
{
  ...
  override void NetworkProcess(ConsoleEvent event)
  {
    if (x5_su.beginsWith(event.name, "x5r"))
...
#+name: wait
#+begin_src elisp :var new-delay=2 :exports none

(setq wait-delay (+ new-delay wait-delay))
(format "wait %d" wait-delay)
#+end_src

#+begin_src text :tangle ../build/10.5xTest/commands.txt

<<init-wait()>>

x5_multiplier 100
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5t_begin

<<wait(9)>>; x5t_name IntegerMultiplier; x5_multiplier 300
<<wait(9)>>; x5t_spawn {'DoomImp':'1','ZombieMan':'1'}
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5t_expect{'DoomImp':'3','ZombieMan':'3'}

<<wait(9)>>; x5t_name FractionalMultiplier; x5_multiplier 270
<<wait(9)>>; x5t_spawn {'DoomImp':'10'}
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5t_expect{'DoomImp':'27'}

<<wait(9)>>; x5t_name Divider; x5_multiplier 70
<<wait(9)>>; x5t_spawn {'DoomImp':'10'}
<<wait(9)>>; map map01
<<wait(9)>>; turn180
<<wait(9)>>; +attack
<<wait(9)>>; -attack
<<wait(9)>>; netevent x5t_expect{'DoomImp':'7'}

<<wait(9)>>; x5t_name PerClass; x5_multiplier 0
<<wait(9)>>; x5t_spawn {'DoomImp':'1','ZombieMan':'1'}
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5r{'DoomImp':'300','ZombieMan':'500'}; closemenu
<<wait(9)>>; netevent x5t_expect{'DoomImp':'3','ZombieMan':'5'}

<<wait(9)>>; x5t_name RandomSpawner; x5_multiplier 200
<<wait(9)>>; x5t_spawn {'Archvile':'1'}
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5t_expect{'x5t_Archvile':'2'}

<<wait(9)>>; x5t_name A_SpawnItemEx; x5_multiplier 200
<<wait(9)>>; x5t_spawn {'Revenant':'1'}
<<wait(9)>>; map map01
<<wait(9)>>; netevent x5t_expect{'x5t_Revenant':'2'}

<<wait(9)>>; netevent x5t_end
<<wait(9)>>; quit
#+end_src
