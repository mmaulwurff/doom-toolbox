...
- Actors that are normally friendly, but changed their friendliness by scripts, don't
  count as targets.
- Guncaster and Guncaster Vindicated aren't compatible with Typist.pk3. Reason:
  Guncaster reads player input directly from player, so weapon firing can't be
