...
class x5t_Test : StaticEventHandler
{
  ...
  override void networkProcess(ConsoleEvent event)
  {
    if (event.name == "x5t_begin")
      ...
      {
        Dictionary expected = Dictionary.fromString(x5t_Quoter.quote(event.name.mid(10)));
  ...
  private void testActorClass(string testName, int expectedCount, string actorClassName)
  {
    int aliveCount = 0;
    int canMoveCount = 0;
    ...
    mTest.it(description .. ": alive", mTest.AssertEval(aliveCount, "==", expectedCount));
    mTest.it(description .. ": can move", mTest.AssertEval(canMoveCount, "==", expectedCount));
  }
...
class x5t_RevenantReplacer : Actor replaces Revenant
{
  States
    {
    Spawn:
      TNT1 A 0 { bThruActors = 1; bCountKill = 0; }
