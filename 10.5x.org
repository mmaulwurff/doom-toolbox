...
* Tests
TODO: fix: sometimes the first test fails.
#+begin_src elisp
...
(run-tests
 ...
 "set x5_multiplier 100; wait 2; map map01; wait 2; netevent x5t_begin; wait 2;
  ...
  set x5_multiplier 0;
  x5t_spawn {'DoomImp':'1','ZombieMan':'1'}; wait 2; map map01; wait 10;
  netevent x5r_{'DoomImp':'300','ZombieMan':'500'}; closemenu; wait 10;
  x5t_expected {'DoomImp':'3','ZombieMan':'5'}; netevent x5t_run; wait 10;

  netevent x5t_end; wait 2; quit")
#+end_src

#+begin_src c :tangle build/10.5xTest/mapinfo.txt
GameInfo { EventHandlers = "x5t_Test", "x5t_Resender" }
#+end_src
...
class x5t_Test : StaticEventHandler
{
  ...
  override void WorldLoaded(WorldEvent event)
  {
    ...
    Dictionary spawn = Dictionary.fromString(jsonify(x5t_spawn));
  ...
  static string jsonify(string input)
  {
    input.replace("'", "\"");
    return input;
  }
  ...
  private void run()
  {
    Dictionary expected = Dictionary.fromString(jsonify(x5t_expected));
...
class x5t_Resender : EventHandler
{
  override void NetworkProcess(ConsoleEvent event)
  {
    if (event.name.left(4) == "x5r_")
      {
        sendNetworkEvent("x5_" .. x5t_Test.jsonify(event.name.mid(4)));
      }
  }
} // class x5t_Resender
#+end_src
