...
class x5_EventHandler : EventHandler
{
  ...
  override void worldLoaded(WorldEvent event)
  {
    ...
    {
      mTypeMultipliers = fillTypeMultipliers(mEnemyTypes, x5_multiplier);
    }
  }
  ...
  private static Dictionary fillTypeMultipliers(Dictionary enemyTypes, int multiplier)
  {
    let result              = Dictionary.create();
  ...
  // TODO: don't clone the actor, clone its replacee.
  private static void clone(Actor original)
  {
    let spawned = Actor.spawn(original.getClassName(), original.pos);

    if (!original.bThruActors)
    {
      // for debugging:
      //spawned.a_SetRenderStyle(1, STYLE_Stencil);

      spawned.bThruActors = true;
      spawned.a_GiveInventory('x5_WalkAbilityWatcher', 1);
    }
